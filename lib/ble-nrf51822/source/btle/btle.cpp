...
// Payload + overhead per packet + final packet
static const unsigned int WRITE_BUFFER_SIZE = MAX_ATTR_SIZE + MAX_PACKETS_FOR_ATTR*6 + 2;
...
static void btle_handler(ble_evt_t *p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id) {
        ...
        case BLE_EVT_USER_MEM_REQUEST:          
            mem_block.len = WRITE_BUFFER_SIZE;
            mem_block.p_mem = &write_buffer[0];
            (void)sd_ble_user_mem_reply(gap.getConnectionHandle(), &mem_block);
            break;
